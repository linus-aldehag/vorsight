...
public class ShutdownCoordinator(ILogger<ShutdownCoordinator> logger) : IShutdownCoordinator
{
    ...
    public async Task ShutdownGracefullyAsync(TimeSpan? timeout = null)
    {
        ...
        logger.LogInformation(
            "Beginning graceful shutdown, waiting up to {Timeout} seconds for uploads to complete",
    ...
    private async Task WaitForQueueEmptyAsync(BufferBlock<string> queue, TimeSpan timeout)
    {
        ...
        if (initialCount > 0)
        {
            ...
            await using var timer = new Timer(
                state =>
                {
                    if (queue.Count != 0 && !queue.Completion.IsCompleted)
                        return;
            ...
            {
                logger.LogWarning(
                    "Timed out waiting for upload queue to empty. {RemainingCount} items left",
                    queue.Count
                );
                throw new TimeoutException(
                    $"Timed out waiting for upload queue to empty. {queue.Count} items left"
                );
            }
