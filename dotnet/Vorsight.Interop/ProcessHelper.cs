...
public class ProcessHelper(ILogger<ProcessHelper> logger) : IProcessHelper
{
    ...
    )
    {
        if (!ProcessInterop.OpenProcessToken(processHandle, desiredAccess, out tokenHandle))
        {
            ...
            logger.LogWarning("OpenProcessToken failed: {ErrorCode}", err);
            return false;
        }
        return true;
    }
    ...
    )
    {
        ...
        {
            var err = Marshal.GetLastWin32Error();
            logger.LogWarning("DuplicateTokenEx failed: {ErrorCode}", err);
            return false;
        }
        return true;
    }
    ...
    public bool TryOpenProcess(uint processId, uint desiredAccess, out IntPtr processHandle)
    {
        ...
        {
            var err = Marshal.GetLastWin32Error();
            logger.LogWarning("OpenProcess failed: {ErrorCode}", err);
            return false;
        }
        return true;
    }
    ...
    )
    {
        ...
        try
        {
            ...
            {
                var err = Marshal.GetLastWin32Error();
                logger.LogWarning("CreateEnvironmentBlock failed: {ErrorCode}", err);
            ...
            {
                var err = Marshal.GetLastWin32Error();
                logger.LogError("CreateProcessAsUser failed: {ErrorCode}", err);
                return false;
            }
    ...
    )
    {
        ...
        {
            var err = Marshal.GetLastWin32Error();
            logger.LogWarning("WTSQuerySessionInformation failed: {ErrorCode}", err);
            return false;
        }
    ...
    public bool TryEnumerateSessions(out SessionInterop.WTS_SESSION_INFO[]? sessions)
    {
        ...
        {
            var err = Marshal.GetLastWin32Error();
            logger.LogWarning("WTSEnumerateSessions failed: {ErrorCode}", err);
            return false;
        }
